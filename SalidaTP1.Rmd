---
title: "Trabajo Práctico 1 - Parte 1 - WELCOME LICENCIADA v2"
...
Resulta muy complicado visualizar algo... como podemos saber si hay valores extremos ???
...
```{r}
plot(glx_reducido$Nr, glx_reducido$ApDRmag,
    pch=19, xlab="Nro Id Galaxia", ylab="ApDRmag",
    main="Grafico ApDRmag")

```

Se procede a eliminar la fila que tieneese punto anomalo

```{r}
which.min(glx_reducido$ApDRmag) 
glx_reducido <- glx_reducido[- which.min(glx_reducido$ApDRmag) , ]
...
plot(glx_reducido$Nr, glx_reducido$Mcz,
    pch=19, xlab="Nro Id Galaxia", ylab="ApDRmag",
    main="Grafico Mcz")
```
...
p2 <- qplot(BbMAG, S280MAG-BbMAG, data = glx_reducido)
grid.arrange(p1, p2, ncol=2)
...
```{r Gráfico - VI}
glx_reducido <- glx_reducido[- which.max(glx_reducido$BjMAG) , ]
...
glx_reducido <- glx_reducido[-c (which.max(glx_reducido$S280MAG-glx_reducido$UjMAG),which.min(glx_reducido$usMAG)), ]


p4<-qplot(UjMAG,S280MAG-UjMAG, data = glx_reducido)
p5 <-qplot(usMAG,S280MAG-usMAG, data = glx_reducido)
p6 <- qplot(UbMAG,S280MAG-VnMAG, data = glx_reducido)
grid.arrange(p4, p5, p6, ncol=3)
...
# como saco los cinco menores de S280MAG-VjMAG
